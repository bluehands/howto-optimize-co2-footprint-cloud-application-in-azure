...
request_reply = pd.read_csv('./data/exports/requests.csv')

messaging = pd.concat([
    pd.read_csv('./data/exports/requests_messaging_1.csv'),
    pd.read_csv('./data/exports/requests_messaging.csv')
])

messaging.rename(columns={"_ResourceId": "name"}, inplace=True)
request_reply.rename(columns={"_ResourceId": "name"}, inplace=True)

messaging['name'] = messaging['name'].apply(rename_ids)

request_reply['name'] = request_reply['name'].apply(rename_ids)
...
plot_dependencies(messaging, "Bearbeitungsdauer der Requests bei temporal entkoppelter Kommunikation", 1600, 1600,
                  "C:/Development/DHBW/dhbw-bachelorarbeit/assets/images/requests/tempentkoppelt.png")
plot_dependencies(request_reply, "Bearbeitungsdauer der Requests bei temporal gekoppelter Kommunikation", 1600, 1600,
                  "C:/Development/DHBW/dhbw-bachelorarbeit/assets/images/requests/tempgekoppelt.png")
